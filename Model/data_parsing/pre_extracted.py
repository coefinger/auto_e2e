...
def _decode_image(data, return_raw: bool = False):
    img = Image.open(io.BytesIO(data)) if isinstance(data, bytes) else data
    transformed = _TRANSFORM(img)
    if return_raw:
        # Convert RGB to BGR for OpenCV
        raw_bgr = np.array(img)[..., ::-1].copy()
        return transformed, raw_bgr
    return transformed
...
def _decode_sample(sample: dict, return_visualization_image: bool = False) -> dict:
    ...
    for k in cam_keys:
        if return_visualization_image:
            tensor, raw_img = _decode_image(sample[k], return_raw=True)
            frames.append(tensor)
            viz_frames.append(raw_img)
        else:
            frames.append(_decode_image(sample[k]))
